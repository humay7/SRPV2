...
"""with open('humay_params.json', 'r') as f:
    params = json.load(f)"""
...
def single_run(train_set, run_number, results_writer):
    ...
    for epoch in range(1, epochs + 1):
        train_loss, train_accuracy = train_model(model, labeled_loader,unlabeled_loader, optimizer, criterion)
...
def train_model(model, labeled_loader, unlabeled_loader, optimizer, criterion):
    model.train()
    ...
    for images, labels in labeled_loader:
        ...
        if len(images) == 1:
            print("Single-element batch found!")  # Debugging message
            continue  # Skip this batch if necessary
        optimizer.zero_grad()
        images = images.cuda()
        labels = labels.cuda()
        ...
        source_features = model.feature_extractor(images)
        target_features = model.feature_extractor(unlabeled_images)
        adaptation_loss = coral_loss(source_features, target_features)
        #Coral Block ends 
        outputs = model(images)
        loss = criterion(outputs, labels) + domain_loss_coefficient*adaptation_loss
        loss.backward()
        optimizer.step()
