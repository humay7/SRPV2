    ...
    def generate_adversarial_images(self): 
        ...
        for t in range(self.train_iters):  # Main training loop
            if ((t + 1) % self.T_min == 0) and (counter_k < self.k):  # Generate adversarial images
                ...
                # Create a new dataset with only adversarial samples
        ...
        # Return only the adversarial dataset
        return adv_dataset
...
# def show_adversarial_images(adv_images, labels, num_images=8):
#     """
#     Display several adversarial images with their labels.
...
#         # Convert tensor to numpy for plotting, assuming images are normalized for MNIST
#         image = adv_images[i].squeeze().permute(1, 2, 0).cpu().numpy()  # Convert to HWC format
        
#         # Adjust normalization if needed (this assumes MNIST normalization parameters)
#         image = (image * 0.3081 + 0.1307).clip(0, 1)
...
# Generate adversarial images
adv_dataset = train_ops.generate_adversarial_images()

# Load a batch of adversarial images to visualize
adv_loader = DataLoader(adv_dataset, batch_size=8, shuffle=True)
adv_images, adv_labels = next(iter(adv_loader))

# Display adversarial images
show_adversarial_images_save(adv_images, adv_labels, num_images=8)
